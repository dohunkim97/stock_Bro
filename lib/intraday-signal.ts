...
// 09-16 밤~2026-09-21). 실측으로 확정된 원인: 이 계정(Hobby 요금제)은
// schedule의 시/분 필드에 범위나 간격("0-6", "*/10" 등)을 쓴 크론을
// 등록 자체를 거부한다 — 개수 문제가 아니어서(vercel.json 참고 —
// intraday-monitor를 정시 고정값으로 7번 따로 등록해서 해결) 코드 쪽은
// "여러 번 불려도 된다"는 가정을 그대로 유지한다 — runIntradayCycle이
// 매 호출마다 현재 시각으로 "지금이 스캔 구간인지"를 알아서 판단한다.
