...
import { MAX_ENTRIES } from "@/lib/market-data";
import type { DailyEntry } from "@/app/generated/prisma/client";

const GRID_COLS = "26px 1fr 54px 78px 74px 96px 18px";
  ...
  return (
    ...
    >
      {labels.map((l, i) => (
        <span
          key={i}
          style={i === 2 ? { textAlign: "center" } : i >= 3 ? { textAlign: "right" } : undefined}
        >
          {l}
        </span>
      ))}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        ...
        <section style={panelStyle}>
          ...
          <ColHeaders labels={["#", "종목", "시장", "현재가", "등락률", "거래량"]} />
          {volumeEntries.map((s) => (
            <div
              key={s.id}
              className="hover-row"
          ...
          {volumeEntries.length < MAX_ENTRIES && (
        ...
        <section style={panelStyle}>
          <div style={headerRowStyle}>
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              ...
              <span style={{ fontFamily: "var(--mono)", fontSize: 11, color: "var(--up)" }}>▲ TOP {MAX_ENTRIES}</span>
            </div>
            <span style={{ fontSize: 11.5, color: "var(--faint)" }}>등락률순</span>
          </div>
          <ColHeaders labels={["#", "종목", "시장", "현재가", "등락률", "섹터"]} />
          {gainerEntries.map((s) => (
            <div
              key={s.id}
              className="hover-row"
          ...
          {gainerEntries.length < MAX_ENTRIES && (
