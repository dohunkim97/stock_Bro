...
// 감시해서 올려주는 CompanyAnalysis 레코드를 그대로 화면에 펼친다. 회사마다
// category_N_.../sub_N_... 키 구성이 조금씩 달라서(사업요약 클릭 모달 등
// 다른 곳처럼 고정 스키마로 안 만들고) 여기서 최대한 유연하게 순회한다 —
// sub_title/story는 항상 있다고 보고, 그 사이의 stats류/verdict류 필드는
// 이름을 안 가리고 있는 대로 전부 줄로 보여준다.
...
type Category = { category_title?: string; [key: string]: unknown };

function isSubItem(v: unknown): v is SubItem {
...
function CategoryBlock({ category }: { category: Category }) {
  ...
  return (
    <div>
      {title && (
        <div style={{ fontWeight: 800, fontSize: 13, marginBottom: 4, color: "var(--accent)" }}>{title}</div>
      )}
      <div>
        {subItems.map(([k, item]) => (
...
export async function CompanyAnalysisSection({ code }: { code: string }) {
  ...
  const categories = Object.entries(parsed).filter(
    ([k, v]) => k.startsWith("category_") && isSubItem(v)
  ) as [string, Category][];
  const verdicts = Array.isArray(parsed.master_analyst_final_verdict)
    ? (parsed.master_analyst_final_verdict as unknown[]).filter((v): v is string => typeof v === "string")
    : [];
    ...
    <section style={panelStyle}>
      <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 14, flexWrap: "wrap" }}>
        ...
        <a
          ...
          style={{ fontFamily: "var(--mono)", fontSize: 10.5, color: "var(--faint)", textDecoration: "none" }}
        >
          출처: {row.reportName} · DART 원문 보기 ›
        </a>
      </div>
